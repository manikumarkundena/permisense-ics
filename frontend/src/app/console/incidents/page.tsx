...
export default function IncidentsPage() {
  ...
  const load = () => api<Incident[]>("/api/incidents").then(setIncidents).catch((e) => setError(e.message));
  useEffect(() => { load(); }, []);
  ...
  return <ConsoleShell>
    ...
    <div className="incident-list">
      ...
      incidents.map((incident) => <Link className="incident-row" href={`/console/incidents/${incident.correlation_id}`} key={incident.correlation_id}>
        ...
        <div className="incident-meta"><span>{incident.asset_id}</span><span>{new Date(incident.timestamp).toLocaleString()}</span></div>
        <ArrowRight size={16}/>
      </Link>)}
    </div>
  </ConsoleShell>;
